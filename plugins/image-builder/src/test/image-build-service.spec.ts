...
describe('@jupyter-ml/image-builder', () => {
  describe('ImageBuildService', () => {
    ...
    it('reads, creates, and saves Dockerfiles through encoded workspace paths', async () => {
      ...
      expect(getRequest).toHaveBeenCalledWith('jupyter-ml/images/dockerfiles?path=images%2Fbase%20Dockerfile');
      expect(postRequest).toHaveBeenCalledWith(
        'jupyter-ml/images/dockerfiles',
    ...
    it('starts, controls, and registers image builds using encoded build identifiers', async () => {
      ...
      expect(getRequest).toHaveBeenCalledWith('jupyter-ml/images/builds/build%2Fone/logs');
      expect(postRequest).toHaveBeenNthCalledWith(
        1,
        'jupyter-ml/images/builds',
        JSON.stringify({
          dockerfile_path: 'Dockerfile',
          image_reference: 'registry.example.com/team/image:latest',
          credential_source: 'user',
      ...
      expect(postRequest).toHaveBeenNthCalledWith(2, 'jupyter-ml/images/builds/build%2Fone/stop', '{}');
      expect(postRequest).toHaveBeenNthCalledWith(3, 'jupyter-ml/images/builds/build%2Fone/push', '{}');
      expect(postRequest).toHaveBeenNthCalledWith(
        4,
        'jupyter-ml/images/builds/build%2Fone/runtime-image',
    ...
    it('manages personal credentials without adding tokens to summaries', async () => {
      ...
      expect(getRequest).toHaveBeenCalledWith('jupyter-ml/images/credentials');
      expect(postRequest).toHaveBeenCalledWith('jupyter-ml/images/credentials', JSON.stringify(credential));
      expect(putRequest).toHaveBeenCalledWith(
        'jupyter-ml/images/credentials/credential%2F1',
        JSON.stringify({ ...credential, token: undefined })
      );
      expect(deleteRequest).toHaveBeenCalledWith('jupyter-ml/images/credentials/credential%2F1');
    });
  });
});
