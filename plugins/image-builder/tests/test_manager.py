...
from jupyter_ml_image_builder.models import RegistryCredential
...
def test_build_command_transitions_to_succeeded_and_collects_output(manager, monkeypatch):
    ...
    monkeypatch.setattr(
        "jupyter_ml_image_builder.manager.subprocess.Popen", lambda command, **kwargs: calls.append((command, kwargs)) or process
    )
...
def test_admin_push_uses_password_stdin_and_redacts_logs(manager, monkeypatch):
    ...
    monkeypatch.setattr(
        "jupyter_ml_image_builder.manager.subprocess.Popen",
...
def test_failed_docker_command_redacts_token_and_preserves_failure(manager, monkeypatch):
    ...
    monkeypatch.setattr("jupyter_ml_image_builder.manager.subprocess.Popen", lambda *_args, **_kwargs: process)
